...
The main differences is that a class-based component will require you to extend from React whereas functional components are basically plain javascript that can pass props. A class-based component can go more in depth in the components lifecycle.

2. When does a componentWillMount function be called? What about a componentDidUpdate?

Component"Will/Did"Mount function will be called when a component first mounts onto the application and only then. Where as a componentDidUpdate function will be called on every time the application state changes.

3. Define stateful logic.

Stateful logic is code within a component that stores, changes any state.

4. What are the three step of creating a successful test? What is done in each phase?

The first step is making sure that the data that you are trying to test exists within the code and that it's referencing it correctly.

The second step is arranging/setting the values you'd like that data to be set to so that you can then conduct the actual test.

The third step, is setting the expectational result of the test. Whether that be if you are trying to get a fail message or a success message.
